...
serve(async (req) => {
  ...
  try {
    ...
    // 5. Skip gracefully if no client email
    if (!job.client_email) {
      console.warn(`send-deposit-request: job ${job_id} has no client_email — skipping email`);
      return new Response(
  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#f9fafb;padding:40px 0">
    <tr><td align="center">
      <table width="560" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:12px;border:1px solid #e5e7eb;padding:40px 48px;max-width:560px">
        <tr><td>
          <p style="margin:0 0 8px;font-size:22px;font-weight:700;color:#111827">Your deposit is ready</p>
          <p style="margin:0 0 24px;font-size:15px;color:#6b7280">Hi ${clientName},</p>
          <p style="margin:0 0 24px;font-size:15px;color:#374151;line-height:1.6">
            <strong>${contractorName}</strong> is ready to start your job
            <strong>${job.name}</strong>. To get things underway, please pay
            your deposit using the link below.
          </p>
    ...
    if (!resendApiKey) {
      console.error('send-deposit-request: RESEND_API not set — skipping email');
      return new Response(
    ...
    const emailRes = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        to: [job.client_email],
        subject: `Your deposit is ready — ${job.name}`,
        html: htmlBody,
      }),
    });
    ...
    if (!emailRes.ok) {
      ...
      console.error(`send-deposit-request: Resend error ${emailRes.status}: ${errBody}`);
      return new Response(
